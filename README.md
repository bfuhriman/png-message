## About
png-message is a CLI application for encoding and decoding ASCII text in PNG images. It uses least significant bit steganography, meaning the least significant bits of each image pixel are overwritten by the bits from the text. By only targeting the least significant bits, we ensure that the image is not visually altered during the encoding process. This program uses Hamming codes for error correction, so any single-bit errors during data transmission will be corrected before outputting the message. Two-bit errors will be detected, but cannot be fixed.
